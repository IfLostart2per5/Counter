...
COMPACTABLESUFFIX = ".tar.gz"
MODES = ("Distribuition", "Usable")
APPNAME = "Counter"
BUILD = "buildtemp"
BUILDPATH = Path(BUILD)
APP = Path(APPNAME)
APPEXEC = Path(F"{APPNAME}")
LOVEEXEC = Path("love")
...
def cleanapp(mode: str) -> None:
    ...
    cleanpath(APPEXEC)
    cleanpath(APP)
    cleanpath(Path(f"{APPNAME}.tar.gz"))
...
def givepermission(file: Path):
    os.system(f"chmod +x {file}")

#hm....
def makefile(name: Path, content: str):
...
def downloadlove(forcedownloadagain: bool=False):
    ...
    filename = "love-11.5"
    url = "https://www.github.com/love2d/love/releases/download/11.5/love-11.5-linux-x86_64.AppImage"
    if "64" in ARCH:
        filename = "love-11.5-linux-x86_64.AppImage"
    else:
        raise Exception(f"Architeture \"{ARCH}\" not supported")
    print("Downloading LÖVE from " + url)
    download(url)
    Path(filename).rename(LOVEEXEC)
...
def build(mode: str, justclean:bool=False):
    ...
    toapp(LOVEEXEC)
    toapp(file)
    makefile(APPEXEC, "#!/bin/bash\nAPPDIR=\"$(dirname \"$(readlink -f \"$0\")\")\"\nexec \"$APPDIR/love\" \"$APPDIR/game.love\" \"$@\"")
    givepermission(APPEXEC)
    registerfile(APPEXEC)
    print(f"Built {APPEXEC.name}")
    toapp(APPEXEC)
    loadconfigs(APP)
    if mode == "Distribuition":
        tarit(f"{APPNAME}.tar.gz", APP)
        registerfile(Path(f"{APPNAME}.tar.gz"))
        cleanpath(APP)
        print("Packaged it")
    clean(mode)
    print("Done!")
